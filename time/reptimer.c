...
//this will receive the timer handle from the api
timer_t timerid;
...
int main(int argc, char *argv[])
{
	...
	//the event that will be sent to us by the timer
	struct sigevent event;
	int chid, coid;

	//for receiving our pulse message
	message_t msg;

	//for configuring the time delay and intrval
	...
	printf("CLOCK_REALTIME Time : %ld \n",tval.tv_sec);

	clock_gettime(CLOCK_MONOTONIC,&tval);

	printf("CLOCK_MONOTONIC Time : %ld \n",tval.tv_sec);


	ClockPeriod(CLOCK_REALTIME,NULL,&cp,0);
	printf("Current Tick size = %u nsec\n",cp.nsec);

	// for receiving messages we need have a chennel
	...
	//this pulse will be attached to the timer when creating
	...
	//delay interval 1.5s
	itime.it_value.tv_sec=5;    //5 sec
	//this has to be explicity set to 0 else time is not correct
	itime.it_value.tv_nsec=0;//0 secs

	//repeat interval 1.2 seconds total
	itime.it_interval.tv_sec=1;		//1 sec
	itime.it_interval.tv_nsec=2e8;  //0.2 secs
