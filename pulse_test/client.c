...
int main(void) {
	...
	server_pid = 130342914; //need to find the PID using pidin
	server_chid = 1;
	...
	printf("a= %d , b= %d\n",incoming_msg.mypayload.a,incoming_msg.mypayload.b);
	int count=10;

	while(count>0)
	{
		printf("Client sending pulse..\n");
		//use priority of calling thread
		MsgSendPulse(coid, -1, PULSE_1, count);
		count--;
		sleep(1);
	}
	return EXIT_SUCCESS;
